...
class _Random(_ModeIterator[Tuple[Any, int, int]]):
    """
    Self implement for random sample a batch from only one dataset
    """
    def __init__(self, iterables: List[Iterable], limits: Optional[List[Union[int, float]]] = None) -> None:
        super().__init__(iterables)
        self._iterator_idx = list(range(len(self.iterables)))  # what would be dataloader_idx
        self._idx = [0] * len(self.iterables) # what would be batch_idx
        self.limits = limits
        self.is_load_all = [False] * len(self.iterables)
        self._iterator_len = [len(iterator) for iterator in self.iterables]
    ...
    def __next__(self) -> Tuple[Any, int, int]:
        n = len(self.iterators)
        if n == 0 or (False not in self.is_load_all):
            raise StopIteration
        # recompute the weight
        _weight = [w if flag==False else 0.0 for flag, w in zip(self.is_load_all, self._iterator_len)]
        _weight = np.array(_weight) / (1.0 * sum(_weight))
        
        _iterator_idx = np.random.choice(self._iterator_idx, p=_weight)
        _idx = self._idx[_iterator_idx]

        # if limits are set, go to the correct iterator
        if self.limits is not None:
            while self.limits[_iterator_idx] <= _idx:
                self.is_load_all[_iterator_idx] = True
                return self.__next__()

        try:
            out = next(self.iterators[_iterator_idx])
            index = self._idx[_iterator_idx]
            self._idx[_iterator_idx] += 1
            # batch, batch_idx, dataloader_idx
            return out, index, _iterator_idx
        except StopIteration:
            # try the different iterator
            self.is_load_all[_iterator_idx] = True
            return self.__next__()
...
_SUPPORTED_MODES = {
    ...
    "random": _CombinationMode(fn=sum, iterator=_Random)
}
...
class CombinedLoader(Iterable):
    ...
    def __next__(self) -> Any:
        ...
        if isinstance(self._iterator, _Sequential) or isinstance(self._iterator, _Random):
            return out
